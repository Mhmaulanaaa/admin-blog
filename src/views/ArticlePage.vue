...
<template>
  <main class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div v-else-if="error" class="rounded-lg bg-red-50 p-6 text-red-700 text-center">
      <h2 class="text-xl font-bold mb-2">Oops!</h2>
      <p>{{ error }}</p>
      <RouterLink to="/" class="mt-4 inline-block font-semibold underline text-red-800">Return to Home</RouterLink>
    </div>

    <article v-else-if="post">
      <!-- Article Header -->
      <header class="mb-10 text-center">
        <div class="mb-4 text-sm font-semibold uppercase tracking-widest text-brand-600">
          {{ post.category }}
        </div>
        <h1 class="mb-6 font-display text-4xl font-extrabold leading-tight text-gray-900 sm:text-5xl">
          {{ post.title }}
        </h1>
        ...
        <div class="mt-8 flex justify-center gap-4">
          <button class="rounded-full bg-[#1DA1F2] px-4 py-2 text-xs font-semibold text-white transition hover:bg-opacity-90">Share on Twitter</button>
          <button class="rounded-full bg-[#1877F2] px-4 py-2 text-xs font-semibold text-white transition hover:bg-opacity-90">Share on Facebook</button>
        </div>
      </header>

      <!-- Featured Image -->
      <div class="mb-12">
        <img :src="post.cover" :alt="post.title" class="max-h-[500px] w-full rounded-2xl object-cover shadow-sm" />
      </div>
    ...
    <!-- Related Posts -->
    <section v-if="latestPosts.length > 0" class="mt-16 border-t border-gray-200 pt-16">
      <h3 class="mb-8 font-display text-2xl font-bold text-gray-900">Read Next</h3>
      <div class="grid gap-6 md:grid-cols-3">
        ...
        >
          ...
          <div class="p-4">
            <h4 class="font-display text-lg font-bold leading-tight text-gray-900 group-hover:text-brand-600">{{ item.title }}</h4>
            <div class="mt-4 text-xs font-medium text-gray-500">{{ item.publishedAt }}</div>
          </div>
        </RouterLink>
      </div>
    </section>
