...
<template>
  <main class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
    <!-- Ad Slot: Top Banner -->
    <div
      class="mb-10 w-full bg-gray-50 border border-dashed border-gray-300 flex flex-col items-center justify-center p-4 min-h-[100px] rounded-xl"
    >
      <span class="text-xs uppercase tracking-widest text-gray-400 mb-2">
        Advertisement
      </span>
      ...
      <AdsterraBanner
        id="adsterra-top"
        :options="{
          key: '3a40c8c1bfda225cb324c4ca54eb7fac',
          format: 'iframe',
          height: 90,
          width: 728,
        }"
        scriptSrc="https://www.highperformanceformat.com/3a40c8c1bfda225cb324c4ca54eb7fac/invoke.js"
      />
    </div>
    ...
    <template v-else-if="posts.length > 0">
      ...
      <section class="mb-16">
        <RouterLink
          ...
          class="group block overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition hover:shadow-md"
        >
          <div class="grid md:grid-cols-2">
            ...
            <div class="flex flex-col justify-center p-8 sm:p-10">
              <div
                class="mb-4 flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-wide text-brand-600"
              >
                <span>{{ featuredPost.category }}</span>
              </div>
              <h2
                class="mb-4 font-display text-3xl font-bold leading-tight text-gray-900 sm:text-4xl"
              >
                {{ featuredPost.title }}
              </h2>
              ...
              <div
                class="mt-auto flex items-center gap-3 text-sm text-gray-500"
              >
      ...
      <div
        class="mb-12 flex items-baseline justify-between border-b border-gray-200 pb-4"
      >
        ...
        <template v-for="(post, index) in remainingPosts" :key="post.id">
          ...
          >
            ...
            <div class="flex flex-1 flex-col p-6">
              ...
              <h3
                class="font-display text-xl font-bold leading-snug text-gray-900"
              >
                {{ post.title }}
              </h3>
              <p
                class="mt-3 line-clamp-2 text-sm text-gray-600 leading-relaxed"
              >
                {{ post.excerpt }}
              </p>
              <div
                class="mt-auto pt-6 flex items-center gap-2 text-xs text-gray-500"
              >
          ...
          <!-- <div
            v-if="index + 1 === 3 || index + 1 === 6"
            class="col-span-full my-4 flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-gray-50 p-4 text-center"
          >
            <span class="text-xs uppercase tracking-widest text-gray-400 mb-2">
              Advertisement
            </span>
      ...
      <div
        v-if="remainingPosts.length === 0 && posts.length > 0"
